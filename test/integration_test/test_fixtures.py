...
class TestFixtures:
    ...
    I2P_TUNNEL_READY_REGEX = r"[\s\S]*I2P: session created"
    ...
    BEP5_PUBK_ANNOUNCE_REGEX = r"HTTP signing public key \(Ed25519\): ([a-zA-Z0-9]+)"
    BEP5_PUBK_ANNOUNCE_REGEX = (
        r"[\s\S]*HTTP signing public key \(Ed25519\): ([a-zA-Z0-9]+)[\s\S]*"
    )
    BEP5_REQUEST_CACHED_REGEX = r"[\s\S]*X-Ouinet-Injection:[\s\S]*"
