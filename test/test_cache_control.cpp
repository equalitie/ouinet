...
template<class F> static void run_spawned(asio::io_context& ctx, F&& f) {
    task::spawn_detached(ctx, [f = forward<F>(f)](auto yield) {
            try {
                ...
                BOOST_ERROR("Test ended with unknown exception");
            }
        });
    ctx.run();
}
...
BOOST_AUTO_TEST_CASE(test_cache_origin_fail)
{
    ...
    cc.fetch_fresh = [&](auto rq, auto ce, auto yield) {
        origin_check++;
        BOOST_CHECK(!ce);
    ...
    run_spawned(ctx, [&](auto yield) {
            ...
            auto req = CacheRequest::from(normal_request, yield).value();
            Cancel cancel;
...
BOOST_AUTO_TEST_CASE(test_max_cached_age)
{
    ...
    run_spawned(ctx, [&](auto yield) {
        ...
        cc.fetch_stored = [&](auto rq, auto y) {
            ...
            return make_entry(created, rs, y);
        };

        cc.fetch_fresh = [&](auto rq, auto ce, auto y) {
            origin_check++;
            BOOST_CHECK(ce);
            BOOST_CHECK_EQUAL(rq.resource_id(), old_resource_id);
            return make_session({http::status::ok, CacheRequest::HTTP_VERSION}, y);
        };

        {
            ...
            auto req = CacheRequest::from(normal_req, yield).value();
            Cancel cancel;
            sys::error_code fresh_ec, cache_ec;
...
BOOST_AUTO_TEST_CASE(test_no_etag_override)
{
    ...
    run_spawned(ctx, [&](auto yield) {
            ...
            auto rq = CacheRequest::from(normal_rq, yield).value();
            Cancel cancel;
            sys::error_code fresh_ec, cache_ec;
...
BOOST_AUTO_TEST_CASE(test_if_none_match)
{
    ...
    cc.fetch_stored = [&](auto rq, auto y) {
        ...
        return make_entry(current_time() - seconds(20), rs, y);
    };
...
BOOST_AUTO_TEST_CASE(test_req_no_cache_fresh_origin_ok)
{
    ...
    cc.fetch_stored = [&](auto rq, auto y) {
        ...
        rs.set("X-Test", "from-cache");
        return make_entry(current_time(), rs, y);
    };
