...
public class Ouinet {
    ...
    public void start() {
        ...
        maybeAdd(args, "--injector-ep",            config.injector_endpoint);
        maybeAdd(args, "--injector-credentials",   config.injector_credentials);
        maybeAdd(args, "--index-bep44-public-key", config.index_bep44_pubkey);
        maybeAdd(args, "--index-ipns-id",          config.index_ipns_id);

        if (config.tls_ca_cert_store_path != null) {
            String ca_cert_path;
            String assetPrefix = "file:///android_asset/";

            if (config.tls_ca_cert_store_path.startsWith(assetPrefix)) {
                ...
                // the C++ code.
                String asset = config.tls_ca_cert_store_path.substring(assetPrefix.length());
                ...
                maybeAdd(args, "--tls-ca-cert-store-path", config.tls_ca_cert_store_path);
            }
        }

        try {
            if (config.injector_tls_cert != null) {
                String cert_path = dir() + "/injector-tls-cert.pem";
                writeToFile(cert_path, config.injector_tls_cert.getBytes());
    ...
    //----------------------------------------------------------------
    private boolean copyAssetToFile(String asset, String path)
    {
        try {
    ...
    //----------------------------------------------------------------
    private String dir() {
        return ctx.getFilesDir().getAbsolutePath() + "/ouinet";
    }
