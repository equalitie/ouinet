...
EMULATOR_IMAGE="system-images;$PLATFORM;$EMULATOR_IMAGE_TAG;$ABI" # The image to be used by the emulator AVD
EMULATOR_DEV=${EMULATOR_DEV:-Nexus 6} # To get list of all devices, use `avdmanager list device`.
...
"
    ...
    export PATH="$SDK_DIR/platform-tools:$SDK_DIR/tools/bin:$SDK_DIR/tools:$PATH"
...
function maybe_create_avd {
    if ! emulator -list-avds | grep -q "^$EMULATOR_AVD$"; then
        echo no | avdmanager create avd \
            -n "$EMULATOR_AVD" \
            -k "$EMULATOR_IMAGE" \
            -g "$EMULATOR_IMAGE_TAG" \
            -b "$ABI" \
            -d "$EMULATOR_DEV"
    fi
}
...
function maybe_install_gradle {
    ...
    if [ $NEED_GRADLE == true ]; then
        ...
        export PATH="${TOOLS_DIR}/${GRADLE}/bin:$PATH"
    fi
}
...
function build_ouinet_aar {
    OUTPUT_DIR="${BUILD_DIR}/build-android-${ABI}-${VARIANT}"
    GRADLE_BUILDDIR="${OUTPUT_DIR}/ouinet"
    OUINET_VERSION_NAME=$(cat "${SOURCE_DIR}"/version.txt)
    OUINET_BUILD_ID=$(cd "${SOURCE_DIR}" && "${SOURCE_DIR}"/scripts/git-version-string.sh)
    BUILD_COMMAND=assemble${VARIANT^}

    mkdir -p "${OUTPUT_DIR}"
    mkdir -p "${GRADLE_BUILDDIR}"
    pushd "${GRADLE_BUILDDIR}" >/dev/null
    gradle ${BUILD_COMMAND} \
        -Pandroid_abi=${ABI} \
        -PversionName="${OUINET_VERSION_NAME}" \
        -PbuildId="${OUINET_BUILD_ID}" \
        -PbuildDir="${GRADLE_BUILDDIR}" \
        --project-dir="${SOURCE_DIR}"/android \
        --gradle-user-home "${GRADLE_BUILDDIR}"/.gradle-home \
        --project-cache-dir "${GRADLE_BUILDDIR}"/.gradle-cache
    popd >/dev/null

    APK="${OUTPUT_DIR}/ouinet/outputs/aar/ouinet-${variant}.aar"
}
...
function run_emulator {
    ...
    emulator -avd "$EMULATOR_AVD" -skin "$EMULATOR_SKIN" -use-system-libs "$@" &
