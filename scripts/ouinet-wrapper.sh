...
if grep -qE '^\s*(cache-index|index-ipns-id|announce-in-bep5-swarm|cache-local-capacity|disable-cache|seed-content)\s*=' "$CONF" && ! has_help_arg "$@"; then
    sed -i -E 's/^(\s*)(cache-index|index-ipns-id|announce-in-bep5-swarm|cache-local-capacity|disable-cache|seed-content)(\s*=.*)/##\1\2\3  # obsolete/' "$CONF"
fi
