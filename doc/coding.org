...
The cache client puts the local cache in the =data-v<N>= subdirectory of the cache directory given on cache client creation (where =N= is the version number of the store format), which is created if still missing. The locations of the static cache (if used) and its content files are arbitrary. The cache client also runs a [[file:../src/cache/client.cpp::struct GarbageCollector][garbage collector]] which iterates periodically over all local cache entries (using the [[file:../src/cache/http_store.h::for_each(keep_func][~HttpStore::for_each~]] method) and invokes a [[file:../src/cache/client.cpp::bool keep_cache_entry(][function]] to determine whether the entry should be kept or removed (e.g. if invalid or past its maximum age).
