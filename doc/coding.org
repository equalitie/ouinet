...
- Optional directories for a static cache and its content (see below).
...
The cache client puts the local cache in the =data-v<N>= subdirectory of the cache directory given on cache client creation (where =N= is the version number of the store format), which is created if still missing. The location of the static cache (if used) is arbitrary.
