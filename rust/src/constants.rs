...
// These are only for (user) testing, for production use the commented out values
pub const INCREMENT_RECORD_VERSION_AFTER: Duration = Duration::from_secs(60 * 3);
pub const ROTATE_DEVICE_ID_AFTER: Duration = Duration::from_secs(60 * 5);
//pub const INCREMENT_RECORD_VERSION_AFTER: Duration = Duration::from_secs(60 * 60 * 24);
